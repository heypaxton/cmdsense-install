...
class ReadyToRange(object):
    ...
    def loop(self):
        ...
        device_position = self.pozyx.setPositionFilter(filter_type=FILTER_TYPE_FIR, filter_strength=15, remote_id=None)
        device_range = DeviceRange()
        status = self.pozyx.doRanging(self.destination_id, device_range, self.remote_id)
        if status == POZYX_SUCCESS:
            print(device_range)
            dweet = requests.post("https://dweet.io/dweet/for/cmdsense-tag", { "distance": device_range.distance })
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    destination_id = 0x6049      # network ID of the ranging destination
